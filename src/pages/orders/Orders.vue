<template>
   <div class="pt-3 pb-2 mb-3 border-bottom">
    <a href="javascript:void(0)" @click="exportCSV()"  class="btn btn-sm btn-outline-secondary">Export</a>
  </div>
        ...
        <template v-for="order in orders" :key="order.id">
          <tr>
            ...
            <td>
              ...
              <a href="javascript:void(0)" class="btn btn-sm btn-outline-danger" @click="select(order.id)" >View</a>
            </div></td>
          </tr>
          <tr>
            <td colspan="5">
              <div class="overflow-hidden" :class="selected === order.id ? 'show': 'hide'">
                ...
                <tbody>
                  <tr v-for="item in order.order_item" :key="item.id">
                  <td>{{ item.id }}</td>
